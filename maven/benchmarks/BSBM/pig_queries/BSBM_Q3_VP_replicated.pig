...
SET job.name 'BSBM Q3 replicated ON $inputData' ;
...
BGP1 = JOIN t0 BY product, t4 BY product, t3 BY product, t1 BY product, t2 BY product USING 'replicated' ;
...
BGP2 = JOIN t1 BY product, t0 BY product USING 'replicated' ;
BGP2 = FOREACH BGP2 GENERATE t0::product AS product, t1::testVar AS testVar ;

-- OPTIONAL
lj = JOIN BGP1 BY product LEFT OUTER, BGP2 BY product USING 'replicated' ;
