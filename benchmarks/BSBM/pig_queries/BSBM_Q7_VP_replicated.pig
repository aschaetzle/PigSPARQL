...
SET job.name 'BSBM Q7 replicated ON $inputData' ;
...
j2 = JOIN t5 BY (offer,vendor), j1 BY (offer,vendor) USING 'replicated' ;
j2 = FOREACH j2 GENERATE j1::offer AS offer, j1::productXYZ AS productXYZ, j1::price AS price, j1::vendor AS vendor ;
BGP2 = JOIN t3 BY vendor, t4 BY vendor, j2 BY vendor USING 'replicated' ;
...
BGP3 = JOIN t9 BY reviewer, j1 BY reviewer USING 'replicated' ;
